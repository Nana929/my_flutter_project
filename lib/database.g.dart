    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `TodoItem` (`id` INTEGER NOT NULL, `todoItem` TEXT NOT NULL, `quantity` INTEGER NOT NULL, PRIMARY KEY (`id`))');
  ...
  Future<List<TodoItem>> findAllTodoItems() async {
    return _queryAdapter.queryList('Select * from TodoItem',
        mapper: (Map<String, Object?> row) => TodoItem(row['id'] as int,
            row['todoItem'] as String, row['quantity'] as int));
  }
